...
col1, col2 = st.columns([8, 1])  # Divide la pantalla en una proporción 3:1

# En la columna col1 (izquierda), mostrar el contenido
with col1:
    pass  # Aquí puedes colocar el contenido del dashboard

# En la columna col2 (derecha), colocar el botón "Cerrar sesión"
with col2:
...
fig2 = px.line(linechart, x="month_year", y="Sales", labels={
    "Sales": "Amount"}, height=800, width=1500, template="gridon")
...
with chart1:
    st.subheader('Product Name wise Sales')

    fig = px.box(filtered_df, x="Sales", y="Product Name")
...
with chart2:
    ...
    fig = px.pie(filtered_df, values="Sales",
                    names="Sub-Category", template="gridon")
    fig.update_traces(
        text=filtered_df["Sub-Category"], textposition="inside")
    st.plotly_chart(fig, use_container_width=True)
